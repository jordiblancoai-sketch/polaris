...
export default function IndiaMapContent({ onStateSelect, selectedState }: Props) {
  const [hovered, setHovered] = useState<string | null>(null);
  const [tooltip, setTooltip] = useState<{ x: number; y: number; name: string; score: number } | null>(null);

  // Normalize names — strip diacritics ("Mahārāshtra" → "MAHARASHTRA") so they match score data
  const norm = (s: string) =>
  ...
  return (
    <div className="relative w-full h-full bg-gradient-to-br from-navy-950 via-blue-950 to-navy-950 rounded-xl overflow-hidden">
      ...
      <div className="absolute top-6 right-6 z-20 text-[10px] text-white/50">
        🇮🇳 → 🇺🇸 Interactive State Scoring
      </div>

      <ComposableMap
        projection="geoMercator"
        projectionConfig={{ center: [82.75, 22.75], scale: 897 }}
        width={800}
        height={600}
            ...
            geographies.map((geo: any) => {
              ...
              return (
                <Geography
                  ...
                  onClick={() => { if (hasScore) onStateSelect?.(name); }}
                />
              );
            })
          }
